...
- 新規作成画面のフォームから送信されたリクエストデータを、すべて連想配列として取得している。
...
- php artisan migrateコマンドを実行したときに呼び出される。upとdownに関しての動きは以下の通り。
- up : データベースに新しいテーブル、カラム、またはインデックスを追加するために使用
-  down : upメソッドによって実行する操作と逆の操作を実装し、以前の状態へ戻すために使用
### Seederクラスの役割は何か
テストデータを導入したりするときに使用する。デバックを行ったりできる。
Seederクラス内でdd()関数を使用することによって、下記がわかる。


- 型（Type）: 文字列なのか、数値なのか、あるいは特定のクラスのインスタンスなのか
- 構造: 配列の中に何個データが入っていて、それぞれのキーは何なのか。
- リレーション: Laravelのモデルなら、関連付けられた他のテーブルのデータまで展開して見ることができる。
